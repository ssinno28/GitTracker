...
{
    public class CreatedDateValueProvider : IValueProvider
    {
        private readonly IGitRepo _gitRepo;
        private readonly IPathProvider _pathProvider;

        public CreatedDateValueProvider(IGitRepo gitRepo, IPathProvider pathProvider)
        {
            _gitRepo = gitRepo;
        ...
        {
            return propertyInfo.Name.Equals(nameof(TrackedItem.CreatedDate));
        }
        ...
        public async Task<object> GetValue(TrackedItem trackedItem, PropertyInfo propertyInfo)
        {
            ...
            return !commits.Any() ? default(DateTimeOffset) : commits.Min(x => x.Date);
        }
    }
}